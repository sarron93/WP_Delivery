...
function fg_register_metabox() {
	...
	$post_types	= apply_filters( 'fg_post_types', array( 'post', 'crf_portfolio' ) );
	$context	= apply_filters( 'fg_context', 'side' );
	$priority	= apply_filters( 'fg_priority', 'default' );

	foreach ( $post_types as $post_type ) {

		add_meta_box( 'featuredgallerydiv', esc_html__( 'Featured Gallery', 'semona' ), 'fg_display_metabox', $post_type, $context, $priority );
...
function fg_display_metabox() {
	...
	$selectText    = esc_html__( 'Select Images', 'semona' );
	$visible       = ''; //SHOULD WE SHOW THE REMOVE ALL BUTTON? THIS WILL BE APPLIED AS A CLASS, AND IS BLANK BY DEFAULT.
	$galleryArray  = crf_get_post_gallery_ids( $post->ID );
	$galleryString = crf_get_post_gallery_ids( $post->ID, 'string' );
	if ( ! empty( $galleryString ) ) {
		foreach ( $galleryArray as &$id ) {
			$image = wp_get_attachment_image_src( $id );
			if( $image ) {
				$galleryHTML .= '<li>'.$button.'<img id="'.$id.'" src="' . $image[0] . '"></li> ';
			}
		}
		$selectText = esc_html__( 'Edit Selection', 'semona' );
		$visible    = " visible";
	} 
	...
	<button class="button<?php echo esc_attr( $visible.$oldfix ); ?>" id="fg_removeall"><?php _e( 'Remove All', 'semona' ); ?></button>
