...
;(function ( $ ) {
  ...
  var $errorPopup = $('<div id="error-popup"></div>');
    ...
    var $streamsList = $('.section-stream[data-view-mode="streams-list"]');
		var $streamsUpdate = $('.section-stream[data-view-mode="streams-update"]');
    ...
    $roles.find('option').each(function(){
      rolesStr += '<div class="checkbox-row"><input type="checkbox" value="' +  this.value + '" name="stream-%id%-roles" id="stream-%id%-role-' +  this.value + '"><label for="stream-%id%-role-' +  this.value + '">' + this.innerHTML +'</label></div>';
		...
		console.log(initialStreamCount, nextId)
    ...
    /*$content.find("input[id^=moderation]").each(function(){
      ...
      location.reload();
		...
		$content.on( 'click', '#streams-list .flaticon-trash', function (e) {
			...
			if (!confirm('Are you sure you want to permanatelly delete stream?')) return false;
...
//		console.timeEnd('Change handlers exec')
    ...
    $('#foursquare_client_id, #foursquare_client_secret').change(function(e){
      var $par = $(this).parent();
      if (this.value === '') {
        $par.add($par.prev('dt').first()).hide()
      } else {
        $par.add($par.prev('dt').first()).show()
      }
    }).change()
    ...
    if ($('#instagram_access_token').val() !== '') {
      $("#inst-auth").html('Log Out')
		...
		$content.delegate('input', 'keydown', function (e){
			var $t = $(this)
			if ($t.is('.feed-view-visible input[name$=content]')) {
...
//			$streamsInput.val(JSON.stringify(streamsData));
		...
		$content.find('.restore_backup').click(function(){
			...
			var data = {
				...
				id: $(this).closest('tr').attr('backup-id')
			}
			showLoadingView();
		...
		function setHeight() {
			...
			var maxH;
			//
			$('.section-stream[data-view-mode="streams-update"], #streams-list').each(function(){
				heights.push($(this).outerHeight());
			});

			maxH = Math.max.apply(Math, heights);
			$('.section-content[data-tab=streams-tab]').css('minHeight', maxH);
		}
        ...
        nextId++;
          ...
          streamsData['id' + response.id]['errors'] = response.errors;
debugger;
...
//				console.log(e.type, e.target)
        ...
        $t.on('mouseenter', '.td-status', function(){
          ...
          $errorPopup.html('<h4>Plugin received next error from network API while requesting feed:</h4><p>' + errorStr + '</p>')
        }).on('mouseleave', '.td-status', function(e){
...
//				var obj = optionsToObj($view);
		...
		setTimeout(function(){
			...
			if (activeMode && activeMode.indexOf('update') === -1) {
				$overlay.removeClass('loading');
				$html.addClass('page-loaded');
			}
