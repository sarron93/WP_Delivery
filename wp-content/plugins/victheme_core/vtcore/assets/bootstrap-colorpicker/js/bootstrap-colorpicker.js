...
(function ($) {
  ...
  $(document)
    .on('ready.bscolorpicker ajaxComplete.bscolorpicker', function () {

      // Auto booting using relaxed initialization by queuing the init method
      var objectQueue = $({});

      // Initialize not initialized elements
      $('.bootstrap-colorpicker').each(function () {
        var self = $(this);

        if (typeof self.data('colorpicker') == 'undefined') {

          objectQueue.queue('colorPicker', function (next) {
            setTimeout(function () {
              self
      ...
      objectQueue.dequeue('colorPicker');

    })

    .on('tablemanager-addrow.bscolorpicker', function (events, clone) {
