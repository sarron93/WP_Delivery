...
/**
 * Class for building icon picker select box
 * that falls back to normal select element
 * when no javascript is enabled.
 *
 * Use the icons context array :
 *
 * family     : the icon unique font family name
 * asset      : the vtcore bootstrap asset name for loading the icon fonts asset
 * iconset    : available icon for the fontset
 * base       : css base class nam
 * prefix     : css prefix for each of the iconset icon
 * element    : html tag for the icon
 *
 * by default this form will fallback to fontawesome icon set.
 *
 * @author jason.xie@victheme.com
 * @see VTCore_Form_Interface interface
 */
class VTCore_Bootstrap_Form_BsIcon
...
implements VTCore_Form_Interface {
  ...
  protected function buildOptions() {
    ...
    // Only inject default font icons when no context icons found!
    if (!$this->getContext('icons')) {
      $this->addContext('icons', array(
        'family' => 'fontawesome',
        'asset' => 'font-awesome',
    ...
    if (!isset(self::$library[$this->getContext('icons.family')])) {

      // Load the fontawesome font CSS
      VTCore_Wordpress_Utility::loadAsset($this->getContext('icons.asset'));

      $this->options = array(
        FALSE => __('No Icon', 'victheme_core'),
      );

      if ($this->getContext('icons.iconset')) {
        $icons = array_filter(array_unique(explode(',', str_replace(array(' ', "\n"), array('', ''), $this->getContext('icons.iconset')))));
      }
      ...
      self::$library[$this->getContext('icons.family')] = $this->options;
    }

    if ($this->getContext('limit')) {
      $this->addContext('attributes.data-limit', $this->getContext('limit'));
    }

    $this
      ->addContext('options', self::$library[$this->getContext('icons.family')])
      ->addContext('attributes.data-base', $this->getContext('icons.base'))
      ->addContext('attributes.data-prefix', $this->getContext('icons.prefix'))
      ->addContext('attributes.data-element', $this->getContext('icons.element'));
